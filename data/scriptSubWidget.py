...
class AnimationThread(QtCore.QThread):
    ...
    def run(self):
        ...
        #  so that the method process_blinkRequest() will
        #  be called.

        # self.emit( SIGNAL( "blinkRequest" ) )
...
class ScriptSubWidget(QtWidgets.QWidget):
    def __init__(self, logger, scriptstr, filepath=""):
        ...
        uic.loadUi("data/ui/scriptSubWidget.ui", self)
    ...
    def initTrigger(self):
        ...
        uic.loadUi("data/ui/triggerWidget.ui", self.triggerWidget)
    ...
    def updateTriggerSamplerate(self):
        ...
        #self.config["defaultScriptSampleTime"] = self.logger.scriptSamplerate
    ...
    def closeEvent(self, event):#, *args, **kwargs):
        if self.saved:
            ...
            ok = pyqtlib.alert_message(self.tr("Warnung"),self.tr("Wollen sie die Datei speichern?"), "", "",self.tr("Ja"), self.tr("Nein"))
            if ok != None:
                if ok == True:
                    self.saveScript()
                self.run = False
