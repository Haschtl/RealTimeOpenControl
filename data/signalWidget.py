...
from data.signalEditWidget import SignalEditWidget
import data.define as define
from data.stylePlotGUI import setStyle as setPlotStyle
...
class AnimationThread(QtCore.QThread):
    ...
    def run(self):
        # while  self.thread_must_be_run == True  :
        self.mutex.lock() # locking may not be necessary

        #  Here we emit a signal whenever we want to update window
        #  contents. The other thread will handle this signal
        #  so that the method process_window_update_request() will
        #  be called.
...
class SignalWidget(QtWidgets.QToolButton):
    def __init__(self, plotWidget, logger, devicename, signalname, id, unit):
        ...
        #self.plot.setAlpha(symbol["alpha"], True)
        #randcolor=(random.randint(50, 255), random.randint(50, 255), random.randint(50, 255))

        self.labelItem = pg.TextItem(devicename+"."+signalname, color=symbol["color"], html=None, anchor=(0,0.5))
    ...
    def findStyle(self):
        for signal in self.plotWidget.plotStyles.keys():
            ...
            #pen = pg.mkPen((random.randint(0, 99), 100))
            return symbol, brush
    ...
    def initLabel(self):
        # self.label = AnimatedWidget()
        # hl = QtGui.QVBoxLayout()
        # self.labelx = QtWidgets.QLabel("")
        # hl.addWidget(self.labelx)
        # self.label.setLayout(hl)
    ...
    def updatePlot(self):
        ...
        #self.editWidget.deleteButton.setEnabled(False)
        if len(self.logger.signals) > 0:
            if len(self.logger.getSignal(self.id)[1]) > 0:
                ...
                maxduration = self.calcDuration(list(self.logger.getSignal(self.id)[0]))
                duration = self.logger.getSignal(self.id)[0][-1]-self.logger.getSignal(self.id)[0][0]
                try:
                    line1 = time.strftime("%H:%M:%S", time.gmtime(int(duration))) + \
                        "/~"+time.strftime("%H:%M:%S", time.gmtime(int(maxduration)))
                    line2 = str(
                        len(list(self.logger.getSignal(self.id)[0])))+"/"+str(self.logger.maxLength)
                    count = 20
                    if len(self.logger.getSignal(self.id)[0]) <= count:
                        count = len(self.logger.getSignal(self.id)[0])
                    if count > 1:
                        meaner = list(self.logger.getSignal(self.id)[0])[-count:]
                        diff = 0
                        ...
                        line3 = "? Hz"
                    self.setToolTip(line1+"\n"+line2 + "\n" + line3)
                except:
                    print("Tooltip failed")
            ...
            for idx, event in enumerate(self.logger.getEvents(self.id)[0]):
                ...
                if [event, evtext] not in self.events:
                    ...
                    eventItem = pg.TextItem(evtext, color=(200, 200, 200), html=None,anchor=(0,0.5))
                    ...
                    #self.eventItems[idx].vLine = pg.InfiniteLine(angle=90, movable=False)
            ...
            #if self.id < len(list(self.logger.signals)):
            ...
            if len(clock) > 0:
                ...
                if lastTimestamp >= self.signalTimeOut and self.label.styleSheet() != "background-color: rgb(113, 100, 29)":
                    ...
                    self.label.setStyleSheet("background-color: #232629")
                #elif self.label.styleSheet() == "background-color: rgb(25, 98, 115)":
            #        self.label.setStyleSheet("background-color: #232629")
                # Workaround for XY plot, because Xdata is not clock
                if self.active:
                    ...
                    if clock[len(clock)-1] <= current_time+10000 and clock[len(clock)-1] > current_time-10000:
                        #self.plotWidget.lastUpdate = clock[len(clock)-1]
                    ...
                    for idx, eventItem in enumerate(self.eventItems):
                        ...
                        # eventItem.updateTextPos()
                        self.eventItems[idx].vLine.setPos(pos)
        # try:
        #     self.editWidget.deleteButton.setEnabled(True)
        # except:
        #     print("Deletebutton cannot be disabled, because it does not exist")
    ...
    def remove(self, cb=True, total=True):
        # if len(self.logger.getSignalUnits(self.id))==0:
        #    self.logger.getSignalUnits(self.id).append("")
        self.plotWidget.legend.removeItem(self.legendName)
        if total:
