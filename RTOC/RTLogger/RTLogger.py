...
try:
    ...
    print('Telegram for python not installed. Please install with "pip3 install python-telegram-bot"')
    telegramBot = None
...
class RTLogger(ScriptFunctions):  # , QObject):
    def __init__(self, enableTCP=None, tcpport=None):
        ...
        if self.config['backupIntervall'] > 0:
            self.backupThread = Timer(self.config['backupIntervall'], self.exportJSON,  args=[
                                      self.config['backupFile']])
    ...
    def toggleHTMLPage(self, value=None):
        ...
        if value:
            self.rtoc_web = subprocess.Popen(
                ["bokeh", "serve", self.getDir(__file__)+"/../RTOC_Web.py"], shell=False)
    ...
    def sendTCP(self, hostname="localhost", *args, **kwargs):
        ...
        self.tcpclient._sendTCP(*args, **kwargs)
            ...
            try:
                ...
                if type(msg) == dict and msg != {}:
                    ...
                    if 'getLatest' in msg.keys():
                        ans['latest'] = {}
                        if type(msg['getLatest']) == list:
                            for device in msg['getLatest']:
                                dev = device.split('.')
                                sig = self.getSignal(self.getSignalId(dev[0], dev[1]))
                                ans['latest'][device] = [sig[0][-1], sig[1][-1]]
                        elif msg['getLatest'] == True:
                            for dev in self.signalNames:
                                sig = self.getSignal(self.getSignalId(dev[0], dev[1]))
                                ans['latest']['.'.join(dev)] = [sig[0][-1], sig[1][-1]]
                    if 'getSession' in msg.keys():
                        ans['session'] = self.generateSessionJSON(scripts=None)
    ...
    def getPluginDict(self):
        ...
        for name in self.devicenames.keys():
            ...
            for fun in self.pluginParameters.keys():
                if fun.startswith(name+".") and fun not in [name+".deviceName", name+".close", name+".run", name+".smallGUI", name+".sock", name+".widget", name+".xy", name+".tcppassword", name+".tcpport", name+".devicename", name+".datanames"]:
        ...
        try:
            ...
            if parameter == "get" and type(parameter) == str:
                if type(args[0]) == list:
                    ...
                    exec('self.ret=self.pluginObjects[plugin].'+str(args[0]))
                    return self.ret
                ...
                if type(args[0]) == str:
                    ...
                    strung = str(args[0])
                exec('self.pluginObjects[plugin].'+parameter+" = " + strung)
                self.ret = args[0]
                return self.ret
            else:
...
# Signal functions ############################################################
    ...
    def clearSignals(self, newLength=None):
        ...
        self.events = [[deque([], newLength), deque([], newLength), deque([], newLength),  deque([], newLength),  deque([], newLength)]
                       for _ in range(len(self.signalNames))]
    ...
    def resizeSignals(self, newLength=None):
        ...
        self.events = [[deque(list(self.events[idx][0]), newLength), deque(list(self.events[idx][1]), newLength), deque(list(self.events[idx][2]), newLength), deque(list(self.events[idx][3]), newLength), deque(list(self.events[idx][4]), newLength)]
                       for idx in range(len(self.signalNames))]
    ...
    def __addNewSignal(self, dataY, dataunit, devicename, dataname, dataX=None, createCallback=True):
        ...
        if dataY != None:
            ...
            self.events += [[deque([], newLength), deque([], newLength),
                             deque([], newLength), deque([], newLength), deque([], newLength)]]
            ...
            # self.addNewEvent(text=translate('RTLogger', "Signal-Stream hinzugefugt: ") +
    ...
    def addNewEvent(self,  *args, **kwargs):
        ...
        for idx, arg in enumerate(args):
            ...
            if idx == 4:
                priority = arg
        print("Event: "+str(devicename)+'.'+str(dataname)+": " +
              str(strung)+'\nID: '+str(id)+'\nValue: '+str(value))
        ...
        if [devicename, dataname] not in self.signalNames:
            ...
            self.events += [[deque([], self.maxLength),
                             deque([], self.maxLength), deque([], self.maxLength), deque([], self.maxLength), deque([], self.maxLength)]]
        ...
        self.events[idx][3].append(value)
        self.events[idx][4].append(id)
        if self.newEventCallback:
            self.newEventCallback(x, strung, devicename, dataname, priority, value, id)
        ...
        #    self.callbackEvent()
        if self.config['telegram_bot'] and telegramBot is not None:
    ...
    def __plotNewSignal(self, x, y, dataunit, devicename, dataname, createCallback=False, autoResize=False):
        ...
        self.events += [[deque([], self.maxLength), deque([], self.maxLength),
                         deque([], self.maxLength), deque([], self.maxLength), deque([], self.maxLength)]]
...
# Callback functions ##########################################################
    ...
    def addDataCallback(self, datasY=[], *args, **kwargs):
        ...
        if type(datasY) == list and type(datanames) == list:
            if datasX == [None]:
...
# Other functions #########################################################
    ...
    def exportXLSX(self, filename):
        if xlsxwriter is not None:
            ...
            print('XLSXWriter not installed! Please install with "pip3 install xlsxwriter"')
    ...
    def setBackupIntervall(self, intervall):
        ...
        if intervall >= 0:
            ...
            if intervall > 0:
                self.backupThread = Timer(intervall, self.exportJSON,
                                          args=[self.config['backupFile']])
            return True
        else:
            return False
    ...
    def saveGlobalEvents(self):
        with open(self.config['documentfolder']+"/globalEvents.json", 'w', encoding="utf-8") as fp:
            json.dump(self.globalEvents, fp,  sort_keys=False, indent=4, separators=(',', ': '))

    def saveGlobalActions(self):
        with open(self.config['documentfolder']+"/globalActions.json", 'w', encoding="utf-8") as fp:
